...
#     should then run `aletheia-worker-resume <slug>` to bring the
#     worker back; aletheia-worker-resume prepends the queued mailbox
...
if [[ "$window_alive" -eq 0 ]]; then
    echo "  run: aletheia-worker-resume $slug"
