...
# tmux session (`aletheia-auditor`) so worker/debugger/lightweight
...
# not kill the auditor; reattach by running aletheia-audit again.
#
#   aletheia-audit                                  # boot or attach
#   aletheia-audit "add chapter export, fix search" # boot + initial task
#
...
aletheia-audit() {
    ...
    local session="aletheia-auditor"
    if tmux has-session -t "$session" 2>/dev/null; then
        tmux attach -t "$session"
        return
    fi
...
Initial task: $task"
    ...
    tmux new-session -A -s "$session" -n auditor -e ALETHEIA_ROLE=auditor \
...
# session_ids survive — you can revive them with aletheia-worker-resume
...
aletheia-audit-stop() {
    ...
    if command -v tmux >/dev/null 2>&1; then
        tmux kill-session -t aletheia-workers 2>/dev/null || true
        tmux kill-session -t aletheia-auditor 2>/dev/null || true
    fi
    ...
    if (( hard )); then
        ...
        echo "  reboot with: aletheia-audit"
        echo "  then revive workers with: aletheia-worker-resume <slug>"
    fi
}
