...
# and state files. The next `aletheia-audit` boot will see orphaned
# workers in list-workers and the auditor (or you) can decide whether
# to resume each (`aletheia-worker-resume <slug>`) or cancel them
# (`./scripts/cancel-worker.sh <slug>`).
#
# Scoped to ALETHEIA_ROLE=auditor; non-auditor sessions are no-ops.
...
if command -v tmux >/dev/null 2>&1; then
    ...
    tmux kill-session -t aletheia-dashboard 2>/dev/null || true
fi
...
if [[ -d "$state_dir" ]]; then
    ...
    if [[ "$orphaned_count" -gt 0 ]]; then
        osascript -e "display notification \"$orphaned_count worker(s) orphaned by auditor shutdown\" with title \"Aletheia auditor\"" 2>/dev/null || true
    fi
fi
